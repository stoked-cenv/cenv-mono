...
const { ROOT_DOMAIN, CDK_DEFAULT_ACCOUNT, CDK_DEFAULT_REGION } = process.env;

export const defaultStackProps = {
  env: { account: CDK_DEFAULT_ACCOUNT, region: CDK_DEFAULT_REGION },
}

export const VPC_NAME = process.env.ENV + '-networking';
...
export class ECSServiceStack extends Stack {
  ...
  constructor(params: ECSServiceDeploymentParams) {
    ...
    // domain configuration
      let domainName = rootDomain!;

      if(env==="prod") {
        domainName = `${env}.${process.env.ROOT_DOMAIN}`;
      }

      const subDomain = `${shortName}.${env}`; // i.e. install.dev
    ...
    const logging = new ecs.AwsLogDriver({
      streamPrefix: `${env}-${shortName}-fargate`,
    ...
    const containerName = `${env}-${shortName}-${process.env.CENV_PKG_VERSION}-${shortDigest}`.replace(/\./g,'-');
    ...
    this.loadBalancedFargateService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, `fargate`, {
      cluster: this.cluster, // Required
      assignPublicIp: true,
      loadBalancerName:`${env}-${shortName}-lb`,
      serviceName: `${env}-${shortName}-svc`,
    ...
    this.loadBalancedFargateService.taskDefinition.taskRole?.attachInlinePolicy(
      new iam.Policy(this, `${env}-${shortName}-app-config`, {
        statements: [
