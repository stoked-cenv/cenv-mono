...
export class PythonLambda extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: LambdaProps) {
    ...
    this.lambda_handler = new PythonFunction(this, functionName!, {
      functionName: functionName, description: functionName,// probably needed to allow other Lambda to invoke this Lambda by a mutually known name
      entry: '../src', // required
      ...
      environment: props?.envVars, timeout: props?.timeout || Duration.seconds(30), logRetention: logs.RetentionDays.THREE_DAYS, // this should be set to avoid unlimited disk space usage in docker containers and such.
    });
