...
export async function createRepository(repositoryName: string) {
  try {
    ...
    CenvLog.single.errorLog(`createRepository error: ${CenvLog.colors.errorBold(e as string)}`);
  }
  return false;
}
...
export async function listImages(repositoryName: string) {
  try {
    ...
    CenvLog.single.errorLog(`listImages error: ${CenvLog.colors.errorBold(e as string)}`);
  }
  return false;
}
...
export async function deleteImages(repositoryName: string, imageIds: any = undefined) {
  try {
    ...
    CenvLog.info(` - deleting ecr images ${CenvLog.colors.infoBold(repositoryName)}`);
    ...
    CenvLog.single.errorLog(`deleteImages from ${repositoryName} error : ${CenvLog.colors.errorBold(e as string)}`);
  }
  return false;
}
...
export async function deleteRepository(repositoryName: string, images = false) {
  try {
    ...
    CenvLog.info(` - deleting ecr repo ${CenvLog.colors.infoBold(repositoryName)}`);
    ...
    CenvLog.single.errorLog(`deleteRepository ${repositoryName} error: ${CenvLog.colors.errorBold(e as string)}`);
  }
  return false;
}
...
export async function describeRepositories(): Promise<Repository[] | false> {
  try {
    ...
    CenvLog.single.errorLog(`describeRepositories error: ${CenvLog.colors.errorBold(e as string)}\n${JSON.stringify(e)}`);
  }
  return false;
}
